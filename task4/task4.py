...
if __name__ == "__main__":
    ...
    y_pred = dTModel.transform(X_test)
    y_pred.show()
    y_pred.printSchema()
    ...
    rF = RandomForestClassifier(featuresCol = "features", labelCol = "Labels", numTrees = 10)

    rFModel = rF.fit(X_train)


    y_pred = rFModel.transform(X_test)
    y_pred.show()
    y_pred.printSchema()
    ...
    print()
    print("train-test split = (70-30)%")
    print("classifier : Random Forest, numTrees = 10")
    print()
    print("accuracy of classification, on test set: ",accuracy)
    print()
    print("**********")
